...
class LidlParser:
    ...
    def normalize_price(self, price_list):
        ...
        if price_list:
            ...
            price = re.sub(r'(\d+)\s*.\s*(\d+)', r'\1.\2', price)  # Normalize price format
    ...
    def parse_product_details(self, product_url):
        ...
        try:
            ...
            brand = selector.xpath('//div[@class="brand-details"]//p[@class="brand-name"]//text()').get()
            brand = brand.strip() if brand else ''
            
            regular_price = self.normalize_price(selector.xpath('//del[@class="pricefield__old-price"]//text()').getall())
            selling_price = self.normalize_price(selector.xpath('//strong[@class="pricefield__price"]//text()').getall())
            
            # If regular price is missing, set it to the selling price
            regular_price = regular_price if regular_price else selling_price
            
            price_per_unit = self.normalize_price_per_unit(selector.xpath('//span[@class="pricefield__footer"]//text()').getall())
