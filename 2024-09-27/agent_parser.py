...
class AgentDetailScraper:
    ...
    def parse_agents_descriptions(self, selector):
        """Parse all agents' descriptions"""
        descriptions = selector.xpath("//div[@class='site-about-column']//div[starts-with(@id, 'body-text-2-preview')]/p/text()").extract()
        return ' '.join([desc.strip() for desc in descriptions if desc.strip()])  

    def extract_office_number(self, selector):
        """Extract the office number using regex."""
        office_number_raw = selector.xpath("//div[@class='site-info-contact']/p[3]/text()[3]").extract_first(default='').strip()
        office_number = re.search(r'\(\d{3}\) \d{3}-\d{4}', office_number_raw)  
        return office_number.group(0) if office_number else ''
